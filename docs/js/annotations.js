var Annotations = (function() {
    ...
    function getReviewer() {
        var ri = getEl('reviewer-name');
        return (ri && ri.value.trim()) || 'Anonymous';
    }

    function getAnnotations() {
        var name = App.getCurrentPDFName();
    ...
    function addReply(index, replyText) {
        ...
        if (anns[index].issueNumber) {
            var body = '**' + reply.reviewer + '** replied:\n' + replyText;
            API.addIssueComment(anns[index].issueNumber, body).catch(function() {});
        }
        setAnnotations(anns);
        saveLocal();
        Sidebar.render(getAnnotations());
    ...
    function handleHighlight(pageNum) {
        ...
        showCommentModal('Highlight on page ' + pageNum + ': "' + preview + '"', function(comment) {
            var c = comment || text.substring(0, 100);
            addAnnotation({
                type: 'highlight',
                page: pageNum,
                comment: c,
