...
export function AnnotationSidebar() {
  ...
  var filtered = all
    ...
    .filter(function(a: any) { return statusFilter==='all' || a.status===statusFilter; })
    ...
    React.createElement('div', { className: 'flex-1 overflow-y-auto p-3 flex flex-col gap-2' },
      filtered.length===0
        ...
        : filtered.map(function(ann: any) {
            ...
            var statusCls = STATUS_CLASSES[ann.status] || 'badge-ghost';
            return React.createElement('div', { key:ann.id, className:'card card-compact bg-base-100 border border-base-300 cursor-pointer hover:border-primary hover:shadow-md transition-all' },
              React.createElement('div', { className:'card-body p-2' },
                React.createElement('div', { className:'flex items-center gap-1 text-xs flex-wrap' },
                  React.createElement(IconComponent, { className:'w-3 h-3' }),
                  React.createElement('span', { className:'badge badge-xs '+statusCls }, ann.status||'draft'),
                  ann.page>0 ? React.createElement('span', { className:'text-base-content/50' }, 'Pg '+ann.page) : React.createElement('span', { className:'text-base-content/50' }, 'General'),
                  ann.reviewer ? React.createElement('span', { className:'text-base-content/40 flex items-center gap-0.5' }, React.createElement(User, { className:'w-3 h-3' }), ann.reviewer) : null,
                  React.createElement('button', { className:'btn btn-ghost btn-xs p-0 h-5 w-5 ml-auto text-error', onClick:function(e: any){ e.stopPropagation(); handleDelete(ann.id); }, title:'Delete' }, React.createElement(X, { className:'w-3 h-3' })),
