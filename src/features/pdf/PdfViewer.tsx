...
function SvgPdfPage(props: {
  ...
  useEffect(function() {
    ...
    for (var i = 0; i < globalAnnotations.length; i++) { if (globalAnnotations[i].page === pageNum) { merged.push(globalAnnotations[i]); seen[globalAnnotations[i].id] = true; } }
    for (var j = 0; j < annotations.length; j++) { if (!seen[annotations[j].id]) merged.push(annotations[j]); }
  ...
  var handleTextSelection = function() {
    ...
    var ann = createAnnotation(pageNum, 'highlight', reviewerRef.current, colorRef.current, { id: 'ver-'+Date.now(), timestamp: new Date().toISOString(), objects: objects, updatedBy: reviewerRef.current||'Anonymous' }, anchor, text.substring(0,100), text);
  ...
  var handleMouseDown = function(e: React.MouseEvent) {
    ...
    if (toolRef.current === 'comment') {
      var ann = createAnnotation(pageNum, 'comment', reviewerRef.current, colorRef.current, { id: 'ver-'+Date.now(), timestamp: new Date().toISOString(), objects: [], updatedBy: reviewerRef.current||'Anonymous' }, undefined, '');
      ...
      setEditingNoteId(ann.id); setEditingNoteText(''); return;
    }
  };

  var handleMouseMove = function(e: React.MouseEvent) {
    var pos = getPos(e);
    // FIXED: Dynamic rectangle resizing preview
    ...
    // FIXED: Dynamic resize during drag — update rectCurrent-like state on the annotation itself
    if (resizing) {
      ...
      if (annR && annR.objects && annR.objects[resizing.objIdx]) {
        ...
        if (resizing.handle === 'e') { obj.w = Math.max(20, resizing.origW + dx); }
        if (resizing.handle === 's') { obj.h = Math.max(20, resizing.origH + dy); }
    ...
    if (dragging) {
      ...
      if (annD) {
        ...
        else if (annD.objects && annD.objects[dragging.objIdx]) { annD.objects[dragging.objIdx].x = dragging.origX + dx2; annD.objects[dragging.objIdx].y = dragging.origY + dy2; }
  ...
  var handleMouseUp = function(e: React.MouseEvent) {
    if (isRectDrawing && rectCurrent && rectCurrent.w > 5 && rectCurrent.h > 5) {
      var ann = createAnnotation(pageNum, 'rectangle', reviewerRef.current, colorRef.current, { id: 'ver-'+Date.now(), timestamp: new Date().toISOString(), objects: [{ type:'rectangle', x:rectCurrent.x, y:rectCurrent.y, w:rectCurrent.w, h:rectCurrent.h }], updatedBy: reviewerRef.current||'Anonymous' });
      setAnnotations(annotations.concat([ann])); dispatch({ type:'ADD_ANNOTATION', pdf:currentPDF, payload:ann });
    }
    ...
    if (isDrawing && currentPath) {
      var ann2 = createAnnotation(pageNum, 'drawing', reviewerRef.current, colorRef.current, { id:'ver-'+Date.now(), timestamp:new Date().toISOString(), objects:[{ type:'draw', path:currentPath }], updatedBy:reviewerRef.current||'Anonymous' });
  ...
  var startDrag = function(e: React.MouseEvent, annId: string, objIdx: number) {
    e.stopPropagation(); var pos = getPos(e); var ann = annotations.find(function(a: Annotation) { return a.id===annId; }); if(!ann) return;
  ...
  var startResize = function(e: React.MouseEvent, annId: string, objIdx: number, handle: string) {
    e.stopPropagation(); var pos = getPos(e); var ann = annotations.find(function(a: Annotation) { return a.id===annId; });
  ...
  var saveNoteEdit = function(annId: string) {
    var newAnns = annotations.slice(); var ann = newAnns.find(function(a: Annotation) { return a.id===annId; });
    if(ann) { ann.comment=editingNoteText; ann.objects=[{ type:'note', x:ann.x||0, y:ann.y||0, text:editingNoteText }]; }
    setAnnotations(newAnns); dispatch({ type:'UPDATE_ANNOTATION', pdf:currentPDF, id:annId, changes:{ comment:editingNoteText } }); setEditingNoteId(null);
  };

  var cursorStyle = tool==='select'?'default':tool==='highlight'?'text':tool==='draw'?'crosshair':tool==='rectangle'?'crosshair':tool==='comment'?'cell':'default';

  return React.createElement('div', {
    ref: containerRef, className: 'page-wrapper',
    style: { position:'relative',width:width+'px',height:height+'px',margin:'0 auto',boxShadow:'0 4px 16px rgba(0,0,0,0.6)',background:'white',flexShrink:0,cursor:cursorStyle,userSelect:tool==='select'||tool==='highlight'?'text':'none' },
    onMouseDown: handleMouseDown, onMouseMove: handleMouseMove, onMouseUp: handleMouseUp, 'data-page': pageNum,
  },
    React.createElement('canvas', { ref: canvasRef, style: { display:'block',pointerEvents:'none' } }),
    React.createElement('div', { ref: textLayerRef, style: { position:'absolute',top:0,left:0,right:0,bottom:0,overflow:'hidden',zIndex:2,pointerEvents:tool==='select'||tool==='highlight'?'auto':'none',lineHeight:1.0 }, onMouseUp: handleTextSelection }),
    React.createElement('svg', { style: { position:'absolute',top:0,left:0,width:'100%',height:'100%',zIndex:3,pointerEvents:'auto' } },
      annotations.map(function(ann: Annotation) {
        var objs = (ann.versions&&ann.versions.length>0&&ann.currentVersion>=0) ? ann.versions[ann.currentVersion].objects : ann.objects;
        if(!objs) return null;
        var objColor = ann.color||color; var fillColor = objColor.replace(/[\d.]+\)$/,'0.4)'); var strokeColor = objColor.replace(/[\d.]+\)$/,'1)');
        return objs.map(function(obj: any, idx: number) {
          if(obj.type==='rectangle') return React.createElement('g', { key: ann.id+'-'+idx },
          ...
          if(obj.type==='highlight') return React.createElement('rect', { key:ann.id+'-'+idx, x:obj.x, y:obj.y, width:obj.w, height:obj.h, fill:fillColor, style:{mixBlendMode:'multiply',pointerEvents:'none'} });
          if(obj.type==='draw') return React.createElement('path', { key:ann.id+'-'+idx, d:obj.path, fill:'none', stroke:strokeColor, strokeWidth:3, strokeLinecap:'round', strokeLinejoin:'round', style:{pointerEvents:'none'} });
          return null;
        });
      }),
    ...
    // FIXED: Notes with color-coded border, black text
    React.createElement('div', { style: { position:'absolute',top:0,left:0,width:'100%',height:'100%',pointerEvents:'none',zIndex:4 } },
      annotations.filter(function(a: Annotation) { return a.type==='comment'; }).map(function(note: Annotation) {
        var isEditing = editingNoteId===note.id;
        var noteColor = note.color||color;
        var borderColor = noteColor.replace(/[\d.]+\)$/,'1)');
        return React.createElement('div', {
          key: note.id,
          style: { position:'absolute',top:(note.y||0)+'px',left:(note.x||0)+'px',background:'#fef9c3',border:'2px solid '+borderColor,borderRadius:'2px 8px 8px 8px',padding:'4px 8px',fontSize:'11px',fontFamily:'sans-serif',color:'#1a1a1a',pointerEvents:'auto',maxWidth:'200px',minWidth:'60px',boxShadow:'1px 2px 4px rgba(0,0,0,0.15)',zIndex:5,cursor:'move' },
          ...
          isEditing
            ...
            : React.createElement('div', { style:{ whiteSpace:'pre-wrap',wordBreak:'break-word',minHeight:'16px' } }, note.comment||React.createElement('span',{ style:{ color:'#999',fontStyle:'italic' } },'Double-click to edit'))
        );
      })
    ),
